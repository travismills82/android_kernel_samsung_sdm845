	...
	uint32_t client_handle, struct cam_axi_vote *client_axi_vote)
{
	...
	if ((axi_vote.compressed_bw == 0) &&
		...
		CAM_WARN(CAM_CPAS, "0 vote from client_handle=%d",
			client_handle);
	...
	uint32_t arg_size)
{
	...
	CAM_INFO(CAM_CPAS, "client=%s, streamon_clients=%d",
		soc_private->client_name[client_indx], cpas_core->streamon_clients);
done:
	mutex_unlock(&cpas_core->client_mutex[client_indx]);
	mutex_unlock(&cpas_hw->hw_mutex);
	return rc;
