...
irqreturn_t cam_fd_hw_irq(int irq_num, void *data)
{
	...
	reg_value = cam_fd_soc_register_read(soc_info, CAM_FD_REG_CAMCC,
		0x11C);
	if (reg_value != 0x1) {
		CAM_ERR(CAM_FD, "Fatal: CPAS AHB clk off offset 0x%x val 0x%x",
			0x11C, reg_value);
		return -EFAULT;
	}
	...
	reg_value = cam_fd_soc_register_read(soc_info, CAM_FD_REG_CAMCC,
		0x13C);
	if (reg_value != 0x1) {
		CAM_ERR(CAM_FD, "Fatal: SOC AHB clk off offset 0x%x val 0x%x",
			0x13C, reg_value);
		return -EFAULT;
	}
