	...
	struct cam_hw_soc_info *soc_info)
{
	...
	rc = cam_common_util_get_string_index(soc_info->mem_block_name,
		soc_info->num_mem_block, "cam_cc", &index);
	if ((rc == 0) && (index < CAM_FD_REG_MAX)) {
		soc_private->regbase_index[CAM_FD_REG_CAMCC] = index;
	} else {
		CAM_ERR(CAM_FD, "regbase not found CAMCC, rc=%d, %d %d",
			rc, index, CAM_FD_REG_MAX);
		return -EINVAL;
	}

	CAM_DBG(CAM_FD, "Reg indices : CORE=%d, WRAPPER=%d CAMCC=%d",
		soc_private->regbase_index[CAM_FD_REG_CORE],
		soc_private->regbase_index[CAM_FD_REG_WRAPPER],
		soc_private->regbase_index[CAM_FD_REG_CAMCC]);
