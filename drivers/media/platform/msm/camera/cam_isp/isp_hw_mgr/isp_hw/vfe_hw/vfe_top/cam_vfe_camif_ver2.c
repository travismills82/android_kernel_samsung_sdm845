...
static void cam_vfe_camif_reg_dump(struct cam_isp_resource_node *camif_node)
{
	struct cam_vfe_mux_camif_data        *camif_priv;
	struct cam_vfe_soc_private           *soc_private;
	uint32_t                              val, i;

	camif_priv = (struct cam_vfe_mux_camif_data *)camif_node->res_priv;
	...
	cam_cpas_reg_read(soc_private->cpas_handle, CAM_CPAS_REG_CAMNOC,
		0x420, true, &val);
	CAM_INFO(CAM_ISP, "CAMNOC IFE02 MAXWR_LOW offset 0x%x val 0x%x",
		0x420, val);

	cam_cpas_reg_read(soc_private->cpas_handle, CAM_CPAS_REG_CAMNOC,
		0x820, true, &val);
	CAM_INFO(CAM_ISP, "CAMNOC IFE13 MAXWR_LOW offset 0x%x val 0x%x",
		0x820, val);
}
	...
	void *evt_payload_priv)
{
	...
	case CAM_ISP_HW_EVENT_SOF:
		if (irq_status0 & camif_priv->reg_data->sof_irq_mask) {
			...
			camif_priv->count++;
			if (camif_priv->count == 1)
				CAM_INFO(CAM_ISP, "Vfe %d First SOF received",
					camif_priv->soc_info->index);
